class SearchService {
    ...
    static get _fields() {
        return SearchService._.fields || (SearchService._.fields = 'id,title,publishedCategory(id,name),publishedZip,address(city,state),employmentType,dateLastPublished,publicDescription,isOpen,isPublic,isDeleted');
    }
    ...
    get helper() {
        return this._.helper || (this._.helper = {
                ...
                clearSearchParams: (specificParam) => {
                    ...
                    } else if (specificParam === 'published') {
                        this.searchParams.published.length = 0;
    ...
    get requestParams() {
        return this._.requestParams || (this._.requestParams = {
                ...
                publishedLocation: (isSearch, fields) => {
                    if ('publishedZip' !== fields) {
                        if (this.searchParams.published.length > 0) {
                            var delimiter = isSearch ? '"' : '\'';
                            var equals = isSearch ? ':' : '=';
                            ...
                            for (var j = 0; j < this.searchParams.published.length; j++) {
                                if (!first) {
                                    fragment += ' OR ';
                                } else {
                                    first = false;
                                ...
                                var pub = this.searchParams.published[j];

                                fragment += 'publishedZip' + equals + delimiter + pub + delimiter;
                            }
                ...
                query: (isSearch, additionalQuery, fields) => {
                    ...
                    query += this.requestParams.publishedLocation(isSearch, fields);
    ...
    getCountByPublishedLocation(callback, errorCallback) {
        return this.getCountBy('publishedZip', 'publishedZip', callback, errorCallback);
    }
